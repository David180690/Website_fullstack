<script>
    ...
    let newBeer = {
        ...
        origin: "holland",
        comment: "comment",
        image: "https://waterloominorhockey.com/public/images/common/articles/Tryouts_Clipart.jpg",
        url:"",
    };
...
{#if showAddBeerForm}
    <div class="add-beer-form">
        ...
        <input type="text" placeholder="Image " bind:value={newBeer.image} />
        {#if formErrors.image}<p class="error">{formErrors.image}</p>{/if}
        <input type="text" placeholder="Image URL" bind:value={newBeer.url} />
        {#if formErrors.image}<p class="error">{formErrors.image}</p>{/if}
